...
filterMatrix :: Int -> [Matrix] -> [Matrix]
filterMatrix q [] = []
filterMatrix q m = filterMatrix_r m []
  where
    filterMatrix_r :: [Matrix] -> [Matrix] -> [Matrix]
    filterMatrix_r [] m = m
    filterMatrix_r (x:xs) m
      | xs == [] = if (calcDet x q) == 1 then [x] ++ m else m
      | otherwise = if (calcDet x q) == 1
...
main = do
  args <- getArgs
  let q = read(head args)::Int
  putStrLn $ formatMatrix 8 $ filterMatrix q $ createMatrix $ createRows q
  putStrLn $  "\nThe quotient group {I, -I} has "
              ++ (show $ div (length $ filterMatrix q
              $ createMatrix $ createRows q) 2)
              ++ " elements."
